...
class WidgetPack(object):
  ...
  @abstractmethod
  def build(self, parent, data, choices=None, default=None):
    pass
...
class BaseChooser(WidgetPack):
  ...
  def get_value(self):
    print("{}: got {}".format(self, self.widget.GetValue()))
...
class BaseFileChooser(BaseChooser):
  dialog = None
  default_value_arg = "defaultDir"
  def __init__(self):
    BaseChooser.__init__(self)

  def on_button(self, evt):
    dlg = self.get_dialog()
    print("on_button path={}, id(dlg)={}".format(dlg.GetPath(), id(dlg)))
    result = (self.get_path(dlg)
              if dlg.ShowModal() == wx.ID_OK
              else None)
    if result:
      self.widget.SetValue(result)
      self.default = result # switch default

  def get_path(self, dlg):
    return dlg.GetPath()

  def get_default_value(self):
    def_val = self.dialog.default_value_arg
    if def_val == "defaultPath":
      path = self.last_value
    elif def_val == "defaultDir":
      path = os.path.split(self.last_value)[0]
    elif def_val == "dt":
      dt = self.last_value
      d, m, y = dt.day, dt.month, dt.year
      path = wx.DateTimeFromDMY(d, m - 1, y)
    else:
      path = self.last_value
    return path
  ...
  def get_dialog(self):
    try:
      print("found default: {} in {}".format(self.default, self))
      path = self.get_default_value()
      dialog_args = {self.dialog.default_value_arg: path} if self.last_value else {}
    except Exception as e:
      print("didn't work in {}: {}".format(self, e))
      dialog_args = {}

    dlg = self.dialog(self.parent, **dialog_args)
    return dlg
...
class DirDialog(wx.DirDialog):
  default_args = dict(defaultPath='Select Directory', style=wx.DD_DEFAULT_STYLE)
  default_value_arg = "defaultPath"

  def __init__(self, parent, *args, **kwargs):
    kargs = self.default_args.copy()
    kargs.update(kwargs)
    wx.DirDialog.__init__(self, parent, **kargs)

class FileDialog(wx.FileDialog):
  default_args = dict(defaultFile='Select File', style=wx.DD_DEFAULT_STYLE)
  default_value_arg = "defaultDir"

  def __init__(self, parent, *args, **kwargs):
    kargs = self.default_args.copy()
    kargs.update(kwargs)
    wx.FileDialog.__init__(self, parent, **kargs)
...
def build_subclass(subclass, dialog, default_args=None):
  if default_args:
    return type(subclass, (BaseFileChooser,), {'dialog': dialog, "default_args": default_args})
  return type(subclass, (BaseFileChooser,), {'dialog': dialog})

FileSaverPayload   = build_subclass('FileSaverPayload', FileDialog,
                                    default_args=dict(style=wx.FD_SAVE| wx.FD_FILE_MUST_EXIST,
                                                      defaultFile="Enter Filename"))
FileChooserPayload = build_subclass('FileChooserPayload', FileDialog,
                                    default_args=dict(style=wx.FD_OPEN))
